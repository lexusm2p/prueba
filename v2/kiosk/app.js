// /kiosk/app.js — Seven de Burgers · Kiosko V2 (UX simplificada + gamificación suave)
...
const CURRENCY = 'MXN';
const PRICE_ENDING = 7;           // estética de precios (47, 57, 67, ...)
const DRINK_HINT_DELAY = 2200;    // ms para mostrar nudge de bebidas
const MAX_MINIS_FOR_REWARD = 3;   // gamificación (3 minis → nudge/sonido)

// ===== Estado global =========================================================
const state = {
  catalog: [],        // productos visibles (filtrados)
  allProducts: [],    // catálogo original (para tabs)
  categories: [],     // ['big','mini','combos','otros']
  selectedCat: 'big', // pestaña actual
  cart: [],           // [{id, name, price, qty, options: {...}}]
  hh: null,           // {active, endsAt, ...}
  eta: null,          // {minutes, source}
  readyFeed: [],      // ids/números de orden listos (para feed opcional)
...
function roundEnding(n) {
  // Redondea al entero más cercano que termine en 7
...
app.innerHTML = `
  <header class="header">
    ...
    <div class="row" id="statusRow">
      <span id="etaPill" class="k-badge">ETA — …</span>
      <span id="hhPill"  class="k-badge">Happy Hour</span>
      <button class="btn small ghost" id="btnThemeNext" title="Cambiar tema rápido">Tema</button>
    </div>
  </header>

  <main class="container">
    <div id="tabsBar" class="tabs tabs-sticky" role="tablist" aria-label="Categorías"></div>
      ...
      <div class="modal-foot">
        <div class="total-bar">
          <div class="row" style="gap:8px;">
            <button class="btn small ghost" id="mLess">–</button>
  ...
  ">
    <div class="row">
      <div class="dot"></div>
      <strong style="font-family:var(--font-display)">¿Algo para tomar?</strong>
    </div>
...
// Hook rápido para el tema (carrusel de presets locales si existen)
$('#btnThemeNext')?.addEventListener('click', ()=>{
  safe(async()=>{
    const list = (await safe(()=>window.listThemes?.(), 'listThemes')) || [
      { name:'Base' }, { name:'Pixel Art' }, { name:'Retro Arcade' }, { name:'Y2K' }, { name:'Día de Muertos' }
    ];
...
// ===== Catálogo (demo + carga remota opcional) ===============================
async function loadCatalog() {
  ...
  if (!Array.isArray(products) || products.length === 0) {
    // Fallback mínimo (usa tus ids reales si ya los tienes)
    products = [
      { id:'starter', name:'Starter Burger', price:47, category:'big', icon:'🍔' },
      { id:'koopa',   name:'Koopa Crunch',   price:57, category:'big', icon:'🐢' },
      { id:'fatality',name:'Fatality Flame', price:67, category:'big', icon:'🔥' },
      { id:'hadouken',name:'Hadouken',       price:77, category:'big', icon:'⚡' },
      { id:'final',   name:'Final Boss',     price:97, category:'big', icon:'👑' },

      { id:'starter-mini', name:'Starter Mini', price:27, category:'mini', icon:'🧒' },
      { id:'koopa-mini',   name:'Koopa Mini',   price:37, category:'mini', icon:'🧪' },
      { id:'hadouken-mini',name:'Hadouken Mini',price:47, category:'mini', icon:'🎮' },

      { id:'combo-3minis', name:'Combo 3 Minis', price:97, category:'combos', icon:'🎯' },
    ];
  }
  ...
  const extras = [
    ...
    { id:'pine',  name:'Reb. de Piña', price:8 },
    { id:'ham',   name:'Jamón', price:10 },
...
function renderCards(flash=false) {
  ...
  state.catalog.forEach(p=>{
    ...
    card.className = 'card';
    card.innerHTML = `
      <div class="media"><div class="icon-img" aria-hidden="true" style="display:grid;place-items:center;font-size:48px">${p.icon || '🍔'}</div></div>
  ...
  if (flash) {
    // pequeño feedback visual al cambiar de tab
...
function findLineKey(line) {
  // separa por id + hash de opciones para “mergear” iguales
...
$('#btnCheckout').addEventListener('click', async ()=>{
  ...
  try {
    if (IS_TRAINING) {
      beep?.(); toast?.('Modo PRUEBA: orden simulada ✅');
      clearCart();
      return;
    }
...
function openModal(){
  ...
  // evitar que el fondo se mueva mientras personalizas
...
function updateHHPill() {
  ...
  if (!state.hh || !state.hh.active) {
    el.textContent = 'Happy Hour — inactivo';
    el.className = 'k-badge';
    return;
  }
  el.textContent = 'Happy Hour — activo';
  el.className = 'k-badge ok';
}
function updateETAPill() {
  const el = $('#etaPill');
  const m = state.eta?.minutes;
  if (!m) { el.textContent = 'ETA — …'; return; }
  el.textContent = `ETA — ${m} min`;
...
safe(async()=>{
  if (DB.subscribeHappyHour) {
    DB.subscribeHappyHour(hh=>{
      state.hh = hh;
      updateHHPill();
      // Recalcular totales existentes (solo base? aquí dejamos totales tal cual para no “mutar” carrito a mitad)
...
function openTrackCTA(orderId) {
  ...
  toast?.('Tu pedido está en preparación. Toca aquí para seguirlo.');
