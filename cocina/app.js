// /cocina/app.js
// Vista de cocina: tablero Kanban con 3 columnas.
// - Muestra pedidos con múltiples items (order.items[])
// - "Tomar" solo en PENDIENTE (desaparece para siempre al pasar a IN_PROGRESS)
// - "Listo" solo en IN_PROGRESS
// - "Editar" en PENDIENTE / IN_PROGRESS (edita notas generales)
// - "Entregar" archiva y elimina la tarjeta
...
function renderCard(o){
  // Si viene el formato nuevo con items[], lo mostramos item a item.
  // Si no, hacemos fallback al formato anterior (o.item + o.qty).
  const items = Array.isArray(o.items) && o.items.length
    ? o.items
    : (o.item ? [{ 
        id: o.item.id, name: o.item.name, qty: o.qty || 1, unitPrice: o.item.price || 0,
        baseIngredients: o.baseIngredients || [],
        salsaDefault: o.salsaDefault || null,
        salsaCambiada: o.salsaCambiada || null,
        extras: o.extras || {},
        notes: o.notes || ''
      }] : []);
  ...
  // Acciones: Tomar solo en PENDING; Listo solo en IN_PROGRESS; Editar en PENDING/IN_PROGRESS; Entregar siempre
  const actions = [
...
document.addEventListener('click', async (e)=>{
  ...
  try{
    if (a === 'take'){
      // pasa a EN PREPARACIÓN; al re-render ya no saldrá "Tomar"
    ...
    if (a === 'deliver'){
      ...
      card.remove(); // feedback inmediato
      return;
    }
    if (a === 'edit'){
