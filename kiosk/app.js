...
wrap.innerHTML =    <div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="giftTtl"   style="max-width:520px;width:calc(100% - 24px);background:#0f182a;border:1px solid rgba(255,255,255,.12);border-radius:14px;padding:14px">   <div class="modal-head" style="display:flex;align-items:center;justify-content:space-between;gap:8px">   <h3 id="giftTtl" style="margin:0">🎉 ¡Logro desbloqueado!</h3>   <button id="giftClose" class="btn ghost" aria-label="Cerrar">✕</button>   </div>   <p class="muted" style="margin:6px 0 12px">   Superaste <b>$${Number(state.gift.threshold).toFixed(0)}</b>. ¿Quieres reclamar tu <b>PowerDog Mini</b> gratis?   </p>   <div class="row" style="gap:8px;justify-content:flex-end">   <button class="btn" id="giftAccept">✅ Sí, agregar</button>   <button class="btn ghost" id="giftReject">❌ No, gracias</button>   </div>   </div>;
document.body.appendChild(wrap);
...
cta.id = 'followCta';
cta.style.cssText =    position:fixed;right:12px;bottom:12px;z-index:9998;display:none;   background:#0f182a;border:1px solid rgba(255,255,255,.08);border-radius:12px;   padding:10px;box-shadow:0 10px 24px rgba(0,0,0,.25);max-width:84vw;;
cta.innerHTML =   <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">   <div style="min-width:0">   <div style="font-weight:700">¿Quieres seguir tu pedido?</div>   <div class="muted small" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">   Ábrelo en tu teléfono con un QR o enlace.   </div>   </div>   <div style="display:flex;gap:8px;margin-left:auto">   <button class="btn" id="ctaFollowOpen">Abrir</button>   <button class="btn ghost" id="ctaFollowClose">Cerrar</button>   </div>   </div>  ;
...
card.className='card';
card.innerHTML =   <h3>${it.name}</h3>   <div class="media">   ${iconSrc   ?<img src="${iconSrc}" alt="${it.name}" class="icon-img" loading="lazy"/>  :<div class="icon" aria-hidden="true"></div>}   </div>   <div class="row">   ${(()=>{   const disc = hhDiscountPerUnit(it);   const eff  = Math.max(0, Number(it.price||0) - disc);   return disc>0   ? <div class="price"><s style="opacity:.7">${money(it.price)}</s> <span class="tag">${money(eff)}</span></div>  :<div class="price">${money(it.price)}</div>;   })()}   <div class="row" style="gap:8px">   <button class="btn ghost small" data-a="ing">Ingredientes</button>   <button class="btn small" data-a="order">Ordenar</button>   </div>   </div>;
...
<span class="tag">(+${money(SP)})</span>).join('')}   </div>   </div>   <div class="field"><label>Ingredientes extra</label>   <div class="ul-clean" id="ingrs">   ${extrasIngr.map((obj,i)=>
<input type="checkbox" id="e${i}" ${hasIngr(obj.name)?'checked':''}/>
<label for="e${i}">${obj.name}</label>
<span class="tag">(+${money(obj.price)})</span>).join('')}   </div>   </div>   <div class="field"><label>Cantidad</label>   <input id="qty" type="number" min="1" max="9" value="${qtyVal}"/>   </div>   <div class="field"><label>Comentarios a cocina</label>   <textarea id="notes" placeholder="sin jitomate, poco picante…">${notesVal}</textarea>   </div>;
...
if (t.seconds) return (t.seconds1000) + Math.floor((t.nanoseconds||0)/1e6);
...
const cut = Math.max(1, Math.floor(recent.length0.1));
const trimmed = recent.slice(cut, recent.length - cut);
const avg = trimmed.reduce((a,n)=>a+n,0)/trimmed.length;
etaSmoothed = (etaSmoothed==null) ? avg : (ETA_ALPHAavg + (1-ETA_ALPHA)etaSmoothed);
...
if (q>0){
const bump = Math.min(12, Math.ceil(q1.5));
...
return <li>   <div style="flex:1;min-width:0">   <div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis"><b>${escapeHtml(o.customer||'—')}</b> · ${count} it.</div>   <div class="muted small" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(names)}</div>   </div>   <div class="price">🛎️</div>   </li>;
}).join('');
...
cardBox.innerHTML =   <div style="display:flex;align-items:center;gap:12px">   <div style="width:72px;height:72px;border-radius:12px;background:linear-gradient(135deg,rgba(255,255,255,.08),rgba(255,255,255,.02));   display:grid;place-items:center;font-size:28px">🍔</div>   <div>   <div style="font-weight:800">${roll.title}</div>   <div class="muted small">Palette (${pal}) · rareza: <b>${roll.rarity}</b></div>   </div>   </div>  ;

if (roll.rarity === 'Dorado'){
const code = voucher?.code || '(cupón generado)';
const until = voucher?.expiresAt ? new Date(voucher.expiresAt).toLocaleDateString() : '';
vBox.innerHTML =    <div class="k-card" style="margin-top:8px;padding:10px;border-radius:12px;border:1px dashed rgba(255,255,255,.25)">   <div style="font-weight:700">🎉 ¡Tarjeta Dorada! Cupón -30%</div>   <div class="muted small">Código: <b>${code}</b> ${until?· vence ${until}:''}</div>   <div class="muted small">Canjeable 1 vez. No acumulable con otras promos.</div>   </div>;
} else {
