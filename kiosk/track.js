...
if (typeof DB.subscribeHappyHour === 'function') {
  DB.subscribeHappyHour((hh) => {
    const on = !!hh?.enabled;
    // Usa 'on' para coincidir con estilos del kiosko; cambia a 'hh-on' si tu CSS lo usa.
    hhPill?.classList.toggle('on', on);
    if (hhText) {
      hhText.textContent = on
        ? `Happy Hour – ${Number(hh.discountPercent || 0)}%`
        : 'HH OFF';
    }
  });
}
...
const getPhone = (o) =>
  normPhone(o?.phone ?? o?.meta?.phone ?? o?.customer?.phone ?? '');
...
function renderMine(order) {
  ...
  if (!order) {
    ...
    mineEl.innerHTML =
      '<div class="muted">Escribe tu teléfono y pulsa “Ver estado”.</div>';
    lastMineId = null;
    lastMineStatus = null;
    return;
  }
  ...
  if (st === 'PAID') {
    payLine = `<div class="payline"><b>¡Muchas gracias por tu compra, ${escapeHtml(
      order.customer || 'amig@',
    )}! 💚</b></div>`;
  }
...
function renderReady(list) {
  if (!readyEl) return;
  const rows = (list || [])
    .filter((o) => (o.status || '').toUpperCase() === 'READY')
    ...
    .map((o) => {
      ...
      return `<li>
        <div style="flex:1;min-width:0">
          <div style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis"><b>${escapeHtml(
            o.customer || '—',
          )}</b> · ${count} it.</div>
          <div class="muted" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${names}</div>
        </div>
...
function computeEtaFallback(orders) {
  ...
  for (const o of orders || []) {
    ...
    const ready =
      tsToMs(o.readyAt || o.doneAt || o.timestamps?.readyAt || o.timestamps?.doneAt);
