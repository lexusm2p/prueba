...
// ===== Orders (hotfix aplicado) =====
// Suscripción simplificada (evita '!=' y composites). Ordenamos en cliente.
export const subscribeActiveOrders = (fn) =>
  onSnapshot(query(ordersCol(), where('status','in',['PENDING','IN_PROGRESS','READY'])), fn);

// Útiles extra para Admin / listados
export const subscribeRecentOrders = (fn) =>
  onSnapshot(query(ordersCol(), orderBy('createdAt','desc'), limit(100)), fn);
...
// ===== Inventory / Purchases / Config =====
export const subscribeInventory = (fn) => onSnapshot(inventoryCol(), fn);

export const setInventoryItem = (id, data) =>
  setDoc(doc(db,'inventory',id), data, { merge:true });

export const addPurchase = (row) =>
  addDoc(purchasesCol(), { ...row, createdAt: serverTimestamp() });

export const setConfig = (key, data) =>
  setDoc(configDoc(key), data, { merge:true });

export const getConfig = (key) =>
  getDoc(configDoc(key));
