...
const UNIT = {
  // líquidos
  ml:    { kind: 'vol', toBase: 1, toDisplay: 1 },
  l:     { kind: 'vol', toBase: 1000, toDisplay: 1/1000 },
  cda:   { kind: 'vol', toBase: 15, toDisplay: 1/15 },   // cucharada
  cdta:  { kind: 'vol', toBase: 5,  toDisplay: 1/5  },   // cucharadita
  // sólidos
  g:     { kind: 'mass', toBase: 1, toDisplay: 1 },
  kg:    { kind: 'mass', toBase: 1000, toDisplay: 1/1000 },
...
export function convertQty(q, fromU, toU){
  ...
  if (!f || !t || f.kind !== t.kind || f.toBase == null || t.toDisplay == null) return null;
  const base = q * f.toBase;
  return base * t.toDisplay;
}
...
export function scaleRecipe(recipe, targetMl, opts={}){
  if (!recipe || !recipe.versiones || recipe.versiones.length === 0) {
    throw new Error('scaleRecipe: receta inválida o sin versiones');
  }
